...
class _IntervenantsScreenState extends State<IntervenantsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final heroHeight = (mq.size.height * 0.22).clamp(160.0, 210.0);
    ...
    return Scaffold(
      ...
      body: FutureBuilder<_WorkersPageData>(
        ...
        builder: (context, snapshot) {
          ...
          final allWorkers = data?.allWorkers ?? const [];
          final workers = data != null
              ? _applyFiltersAndSort(allWorkers, _filters, _sortBy)
              : const <_WorkerItem>[];

          return Column(
            children: [
              // 1. Fixed Top Bar (Always pinned at very top)
              Container(
              ...
              // 2. CustomScrollView for collapsible header & list
              Expanded(
                child: RefreshIndicator(
                  ...
                  child: CustomScrollView(
                    slivers: [
                      ...
                      // -- Sticky Department Chips (Sticks below Top Bar) -
                      ...
                      // -- Address / Distance Filter Block (Scrolls away) --
                      SliverToBoxAdapter(
                        child: _buildFilterBlock(data),
                      ),

                      // -- Worker List (Scrolls under sticky chips) -------
                      ..._buildWorkerSlivers(workers, isLoading, hasError, data),
                    ],
                  ),
                ),
              ),
  ...
  List<Widget> _buildWorkerSlivers(
        ...
        SliverPadding(
          ...
          sliver: SliverList(
            delegate: SliverChildBuilderDelegate(
              (context, index) => const _WorkerCardSkeleton(),
              childCount: 4,
            ),
          ),
        ),
    ...
    if (hasError) {
      return [
        SliverToBoxAdapter(
          child: _WorkersErrorCard(onRetry: _refresh),
        ),
      ];
    }
    ...
    if (workers.isEmpty) {
      return [
        SliverToBoxAdapter(
          child: _WorkersEmptyCard(
            onModifyFilters:
                data == null ? _refresh : () => _openFilterSheet(data),
          ),
        ),
      ];
    }
            ...
            (context, index) {
              ...
              return _CompactWorkerCard(
                ...
                onProfileTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) =>
                        IntervenantProfileScreen(workerId: worker.id),
                  ),
                ),
...
class _StickyFilterChipsDelegate extends SliverPersistentHeaderDelegate {
  const _StickyFilterChipsDelegate({
    required this.child,
  ...
  @override
  bool shouldRebuild(covariant _StickyFilterChipsDelegate oldDelegate) {
...
String _resolveWorkerDepartment({
  ...
  if (department.isNotEmpty) {
    // Normalize accented / variant spellings to the canonical values so the
    // Set<String> in _buildDepartmentOptions deduplicates them correctly.
    final norm = department
        .toLowerCase()
        .replaceAll('\u00e9', 'e')
        .replaceAll('\u00e8', 'e')
        .replaceAll('\u00ea', 'e')
