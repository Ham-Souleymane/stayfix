...
class VillaProfileScreen extends StatefulWidget {
  const VillaProfileScreen({super.key});

  @override
  State<VillaProfileScreen> createState() => _VillaProfileScreenState();
}

class _VillaProfileScreenState extends State<VillaProfileScreen> {
  ...
  Future<_ProfilePageData> _loadData(String uid) async {
    ...
    final condoSnap = results[1] as QuerySnapshot<Map<String, dynamic>>;
    final workersSnap = results[2] as QuerySnapshot<Map<String, dynamic>>;
    ...
    if (condoSnap.docs.isNotEmpty) {
      final condoDoc = condoSnap.docs.first;
      condoId = condoDoc.id;
      final condoData = condoDoc.data();
  ...
  Future<void> _withSaving(
    ...
    try {
      ...
      debugPrint('Villa profile save error: $error');
    } finally {
      if (mounted) {
  ...
  Future<void> _updatePhone({
    ...
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;
    var cleanedNational = nationalNumber.replaceAll(RegExp(r'\D'), '');
    final cleanedDial = dialCode.replaceAll(RegExp(r'[^0-9]'), '');
    if (cleanedDial.isNotEmpty && cleanedNational.startsWith(cleanedDial)) {
      cleanedNational = cleanedNational.substring(cleanedDial.length);
    }
    if (cleanedNational.isEmpty || cleanedDial.isEmpty) return;
    final fullPhone = sanitizePhoneNumber('+$cleanedDial$cleanedNational');
  ...
  Future<void> _updateCondo({
    ...
    }, successMessage: 'Villa mise a jour.');
  }
        ...
        builder: (_) => _TogglePreferencesScreen(
          ...
          items: const [
            ...
            _PreferenceToggleItem(
              ...
              subtitle:
                  'Montrer l adresse complete de la villa uniquement si necessaire.',
  ...
  Widget _buildContent(User firebaseUser, _ProfilePageData data) {
    ...
    final rawPhone = _nonEmpty((userData['phone'] as String?)?.trim());
    final phone = rawPhone != null ? sanitizePhoneNumber(rawPhone) : null;
        ...
        SliverToBoxAdapter(
          child: _ProfileHero(
            ...
            onBack: () => Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                builder: (_) => const CondoDashboardScreen(
                  propertyType: 'villa_owner',
                ),
              ),
            ),
        ...
        child: Row(
          ...
          children: [
            ...
            UnreadMessagesNavItem(
              ...
              activeColor: const Color(0xFFD6A85A),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    name?.isNotEmpty == true ? name! : 'Ajouter votre villa',
                    style: GoogleFonts.inter(
                      color: Colors.white,
                      fontSize: 15,
                      fontWeight: FontWeight.w600,
    ...
    final result = await Navigator.push<String>(
      ...
      MaterialPageRoute(
        builder: (_) => GoogleAddressPickerScreen(
          title: 'Choisir l adresse de la villa',
  ...
  Widget build(BuildContext context) {
    return _SheetFrame(
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const _SheetHeader(title: 'Modifier la villa'),
          const SizedBox(height: 16),
          _GoldInput(
                ...
                Row(
                  children: [
                    Expanded(
                      child: _LanguageChoiceChip(
                        label: 'Petit',
                        isActive: themeProvider.fontScale == 0.85,
                        onTap: () => themeProvider.setFontScale(0.85),
                      ),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _LanguageChoiceChip(
                        label: 'Normal',
                        isActive: themeProvider.fontScale == 1.0,
                        onTap: () => themeProvider.setFontScale(1.0),
                      ),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _LanguageChoiceChip(
                        label: 'Grand',
                        isActive: themeProvider.fontScale == 1.20,
                        onTap: () => themeProvider.setFontScale(1.20),
                      ),
                    ),
                  ],
                ),
