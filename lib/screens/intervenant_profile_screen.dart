...
class _IntervenantProfileScreenState extends State<IntervenantProfileScreen> {
  ...
  void _handleChoose() {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text(
          'Passer en StayChat avec cet intervenant ?',
          style: GoogleFonts.inter(
            color: Colors.white,
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              _showSnack('Intervenant passe en StayChat');
            },
            child: Text(
              'Confirmer',
  ...
  void _handleContact() {
    final phone = sanitizePhoneNumber(_data?['phone'] as String?);
    final email = (_data?['email'] as String?)?.trim();
    if (phone.isEmpty && (email == null || email.isEmpty)) {
  ...
  Future<void> _handleWhatsApp() async {
    ...
    // 1. Try primary phone fields
    String rawPhone = (data['phone'] as String?)?.trim() ?? '';
    if (rawPhone.isEmpty) {
      rawPhone = (data['phoneNumber'] as String?)?.trim() ?? '';
    }
    if (rawPhone.isEmpty) {
      rawPhone = (data['telephone'] as String?)?.trim() ?? '';
    }
    if (rawPhone.isEmpty) {
      rawPhone = (data['mobile'] as String?)?.trim() ?? '';
    }

    // 2. Fallback: reconstruct ONLY if national number is non-empty
    if (rawPhone.isEmpty) {
      final dialCode = (data['phoneDialCode'] as String?)?.trim() ?? '';
      final national = (data['phoneNational'] as String?)?.trim() ?? '';
      if (dialCode.isNotEmpty && national.isNotEmpty) {
        rawPhone = '+$dialCode$national'.replaceAll('++', '+');
      } else if (national.length >= 6) {
        rawPhone = national;
      }
    }

    // 3. Fallback: scan fields excluding metadata keys like phoneDialCode or phoneCountryIso
    if (rawPhone.isEmpty) {
      for (final entry in data.entries) {
        final key = entry.key.toLowerCase();
        if (key == 'phonedialcode' || key == 'phonecountryiso') continue;
        if (key.contains('phone') || key.contains('tel')) {
          if (entry.value is String && (entry.value as String).trim().isNotEmpty) {
            rawPhone = (entry.value as String).trim();
            break;
          }
        }
      }
    ...
    // A valid phone number with country/area code must have at least 7 digits
    if (digits.length < 7) {
      _showSnack('Num\u00e9ro non disponible pour cet intervenant.');
      return;
    }

    // Try whatsapp:// scheme first (direct intent — most reliable on Android)
    final waUri = Uri.parse('whatsapp://send?phone=%2B$digits');
    try {
      await launchUrl(waUri, mode: LaunchMode.externalApplication);
      return;
    } catch (_) {
      // ignored — try web fallback
    }
    ...
    try {
      ...
      if (!mounted) return;
      _showSnack('WhatsApp non disponible sur cet appareil.');
    }
  }
  ...
  Widget _buildContent() {
    ...
    final hasVerifications = (quest['authorizedToWorkCanada'] != null) ||
        (quest['isAdult'] != null) ||
        (quest['noCriminalRecord'] != null) ||
        (quest['referredByEmployee'] != null) ||
        (cvReviewAuth == true);
      ...
      slivers: [
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: Row(
              children: [
                Expanded(child: _SecondaryBtn(onTap: _handleContactChat)),
                const SizedBox(width: 10),
                Expanded(child: _WhatsAppBtn(onTap: _handleWhatsApp)),
              ],
            ),
          ),
        ),
        ...
        // Availability card — only shown if worker provided availability slots
          ...
          child: _SectionCard(
            ...
            rows: [
              ...
              _InfoRow(
                  label: 'T\u00e9l\u00e9phone',
                  value: () {
                    final p = sanitizePhoneNumber((data['phone'] as String?));
                    if (p.isNotEmpty) return p;
                    final d = (data['phoneDialCode'] as String?)?.trim() ?? '';
                    final n = (data['phoneNational'] as String?)?.trim() ?? '';
                    if (d.isNotEmpty && n.isNotEmpty) return '+$d$n'.replaceAll('++', '+');
                    return null;
                  }()),
              _InfoRow(
        ...
        // Languages card — only shown if French or English is indicated
        ...
        // Verifications card — only shown if worker filled in verifications
        if (hasVerifications) ...[
          const SliverToBoxAdapter(child: SizedBox(height: 12)),
          SliverToBoxAdapter(
        ...
        // CV card — only shown if CV file is uploaded
        if (cvBytes != null) ...[
          const SliverToBoxAdapter(child: SizedBox(height: 12)),
          SliverToBoxAdapter(
            child: _CvCard(
...
class _WhatsAppBtn extends StatelessWidget {
  const _WhatsAppBtn({required this.onTap});
  final VoidCallback onTap;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: const Color(0xFF25D366).withValues(alpha: 0.55)),
        ),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
...
class _VerificationsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final rows = <Widget>[
      ...
      if (cvReviewAuth == true)
        _VerifRow(
          label: 'Autoris\u00e9 \u00e0 la r\u00e9vision CV',
          value: cvReviewAuth,
