  ...
  Future<_DashboardData> _loadData() async {
    ...
    for (final doc in workerDocs) {
      ...
      final worker = _StoryWorker(
        ...
        stories: const [],
        isAvailableNow: _isWorkerAvailableNow(data),
      );
...
bool _isSlotActiveNow(Map slot, DateTime now) {
  ...
  final startMinute = _parseSlotTimeMinutes(slot['startTime'] ?? slot['start']);
  final endMinute = _parseSlotTimeMinutes(slot['endTime'] ?? slot['end']);
...
bool _isAlwaysAvailableSlot(Map slot) {
  if (slot['isAlwaysAvailable'] == true || slot['alwaysAvailable'] == true) {
    return true;
  }
...
int? _parseSlotTimeMinutes(dynamic value) {
